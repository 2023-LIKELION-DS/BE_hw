...
while(1):
    if num == 1 or num ==2 :
        use_money = int(input('금액: '))
        if use_money > 0 :
            if num == 1:
                money += use_money
                print(use_money,'원이 입금되었습니다. 현재 잔고는 ', money,'원 입니다.\n')
            else :
                money -= use_money
                if money > use_money: 
                    print(use_money,'원이 출금되었습니다. 현재 잔고는 ', money,'원 입니다.\n')
                else : 
                    print('현재 잔고 부족입니다.', abs(use_money),'원이 부족합니다.\n')
        else :
...
board = []
while(1):
    add = input("추가할 메뉴를 입력하세요.(추가 완료 시 '완료'를 입력하세요.)")
    if(add == '완료'):
        break
    else:
        board.append(add)
...
table_num = random.randrange(7)
for i in range(table_num):
    table = random.randrange(1,7)
    menu_choice = random.choice(board)
    print(table,'번째 테이블에서',menu_choice,'주문했습니다.')
...
for i in range(200):
    mbti = ''
    mbti += random.choice(ie)
    mbti += random.choice(sn)
    mbti += random.choice(tf)
    mbti += random.choice(jp)
...
def count_mbti(dic, mbti_type):
    mbti_type = mbti_type.upper()
    if mbti_type in dic:
        count = dic[mbti_type]
        print(f"{mbti_type} 유형은 {count}명이 검사결과로 나왔습니다.")
    else:
        print("유효하지 않은 mbti 유형입니다.")
