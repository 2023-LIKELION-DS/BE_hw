...
while True:
    menu = input("입금이면 1, 출금이면 2 (종료는 아무거나 누르세요): ")
    ...
    if menu == '1':
        ...
        if amount < 0:
            ...
            balance += amount
            print("%d원이 입금되었습니다. 현재 잔고는 %d원 입니다." %(amount, balance))
        ...
        if amount < 0:
            ...
            balance -= amount
            print("{}원이 출금되었습니다. 현재 잔고는 {}원 입니다." .format(amount, balance))
            if balance == 0:
                print("통장을 파기합니다.")
                # break
...
tables = random.randrange(1,7)
for i in range(tables):
    table_number = i + 1
    ordered_menu = random.choice(menu)
    print(f"{table_number}번째 테이블에서 {ordered_menu}를 주문했습니다.")
...
mbti_types = ['ISTJ', 'ISFJ', 'INFJ', 'INTJ', 'ISTP', 'ISFP', 'INFP', 'INTP', 'ESTP', 'ESFP', 'ENFP', 'ENTP', 'ESTJ', 'ESFJ', 'ENTJ', 'ENFJ']

results = []

# for i in range(200):
#     mbti_results = random.choice(mbti_types)
#     results.append(mbti_results)

# print(results)

for i in range(200):
    results.append(random.choice(mbti_types))
...
mbti_results = []
for i in range(200):
    mbti_results.append(random.choice(mbti_types))
    
count_results = {}
for mbti in mbti_types:
    count_results[mbti] = mbti_results.count(mbti)
...
while True:
    user_input = input("mbti 유형을 입력하세요 : ").upper()
    if user_input in mbti_types:
        print(f"{user_input} : {count_results[user_input]}")
        break
    else:
        print("유효한 mbti 유형을 입력하세요.")